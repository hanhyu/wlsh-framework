...
class Server
{
    ...
    {
        $this->server = new Swoole\WebSocket\Server("0.0.0.0", 9770);
    ...
    public function onRequest(Swoole\Http\Request $request, Swoole\Http\Response $response): void
    {
        ...
        if (isset($path_info[1]) AND !empty($path_info[1])) {
            ...
            //请求认证token,过滤掉配置中不需要认证的路由
            $not_auth = \Yaf\Registry::get('router_filter_config')->toArray();
            if ($not_auth[$request->server['path_info']]['auth']) {
                //验证token的合法性与过期时间
                $res = validate_token($request->header);
                if ($res != '0') {
                    $response->end($res);
                    return;
                }
        ...
        //todo 在此处加入路由权限组的钩子方法

        ob_start();
        try {
            $this->obj_yaf->getDispatcher()->dispatch($obj_req);
        } catch (Throwable $e) {
            if ($e->getCode() == 0) { //业务部分手动异常信息
